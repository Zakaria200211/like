'''
اداة رشق فيس م̷ـــِْن موقع
https://t.me/PY_61
'''
#متابعة، إعجاب، إعجاب، إعجاب، تفاعل، تعليق، مشاركة، تفاعل، مجموعة، صفحة
...
class Facebook_Api (object):
	...
	def get_thongtin(self):
		try:
			...
			#تكذو توخر علامة هاشتاك علمود يضهرلك معلومات مثل اسم + ايدي
			#print(f'{luc}اسم حساب: {vang}{ten} {red}| {luc}ايدي: {vang}{self.user_id} ')
...
def error(id, type):
	time=datetime.now().strftime("%H:%M:%S")
	uid = id.split('_')[1] if '_' in id else id
	print(f'[X] | {time} | {type} | {uid} | خطا', end = '\r');sleep(1); print('                                                       ', end = '\r')
...
def Nhap_Cookie():
	...
	while True:
		i += 1
		cookie = str(Write.Input(f'[●] ➩ أدخل كوكيز الفيسبوك ث {i}: ',Colors.green_to_yellow,interval=0.0001))
		if cookie == '' and i > 1:
			break
		...
		if name != 0:
			ten = name[0]
			Write.Print(f'الحساب: {ten}\n',Colors.green_to_red,interval=0.0001)
			#Write.Print(f'اسم الفيسبوك: {ten}')
			list_cookie.append(cookie)
			frive(14)
		else:
			Write.Print('كوكيز في الفيسبوك تموت!  الرجاء إعادة الإدخال !!!\n',Colors.green_to_red,interval=0.0001)
			frive(14)
			i-=1
	return list_cookie
def chongblock(delaybl):
	for i in range(delaybl, -1, -1):
		Write.Print(f'ستتم إعادة تشغيل ميزة مكافحة الحظر النشطة بعد {i} ثانية  ',Colors.green_to_blue,interval=0.0001,end = '\r');sleep(1); print('                                                        ', end = '\r')
...
def main():
	...
	while True:
		if os.path.exists('configtds.txt'):
			...
			data = tds.main()
			try:
				Write.Print(f'[●] ➩ أدخل [1] تعليق الحساب '+data ['user'],Colors.green_to_yellow,interval=0.0001)
				Write.Print(f'\n[●] ➩ أدخل [2] أدخل TDS Access_Token الجديد\n',Colors.green_to_yellow,interval=0.0002)
				chon = str(Write.Input('[●] ➩ اختر ===>: ',Colors.green_to_white,interval=0.0002))
				if chon == '2':
					os.remove('configtds.txt')
				elif chon == '1':
					pass
				else:
					Write.Print('اختيار غير معروف !!!\n',Colors.green_to_red,interval=0.0001);frive(14)
					continue 
			except:
				os.remove('configtds.txt')
		if not os.path.exists('configtds.txt'):
			token = str(Write.Input('[●] ➩ أدخل Access_Token TDS: ',Colors.green_to_yellow,interval=0.0001))
		...
		try:
			...
			xudie = data['xudie']
			user = data['user']
			Write.Print('تم تسجيل الدخول بنجاح\n',Colors.green_to_blue,interval=0.0001)
			break
		except:
			Write.Print('رمز الوصول غير صالح!  حاول مرة اخرى\n',Colors.green_to_red,interval=0.0001)
			os.remove('configtds.txt')
			continue 
	frive(14)
	...
	while True:
		if os.path.exists('Cookie_FB.txt'):
			Write.Print('[●] ➩ أدخل [1] استخدام كوكيز المحفوظة على Facebook\n',Colors.green_to_yellow,interval=0.0001)
			Write.Print('[●] ➩ أدخل [2] أدخل الكوكيز الجديد لفيسبوك\n',Colors.green_to_yellow,interval=0.0002)
			chon = str(Write.Input('[●] ➩ الرجاء إدخال: ',Colors.green_to_yellow,interval=0.0002))
			if chon == '1':
				Write.Print('استرجاع البيانات المحفوظة\n',Colors.green_to_blue,interval=0.0001);sleep(1)
	...
	Write.Print(f'العملات المعدنية الحالية: {xu} \n',Colors.green_to_yellow,interval=0.0001)
	Write.Print(f'Xu die: {xudie} \n',Colors.green_to_yellow,interval=0.0001)
	print(f'[●] ➩ اسم الحساب: {user} ')
	print(f'[●] ➩ العملات المعدنية الحالية: {xu}')
	print(f'[●] ➩ عملة الجزاء: {xudie} ')
	print(f'[●] ➩ رقم الكوكيز: {len(list_cookie)} ')
	frive(14)
	
	Write.Print('[1 : إعجاب — 2 : تعليق — 3 : مشاركة — 4 : رد فعل]\n',Colors.green_to_white,interval=0.0001)
	Write.Print('[5 : متابعة — 6 : صفحة — 7 : REACTCMT — 8 : مجموعة]\n',Colors.green_white,interval=0.0001)
	Write.Print('[●] ➩ يمكن تحديد مهام متعددة (مثال 123...)\n',Colors.green_to_white,interval=0.0001)
	nhiemvu = str(Write.Input('[●] ➩ أدخل رقمًا لتشغيل المهمة: ',Colors.green_to_white,interval=0.0001))
	Delaymin = int(Write.Input('[●] ➩ الحد الأدنى لتأخير الإدخال: ',Colors.green_to_white,interval=0.0001))
	delaymax = int(Write.Input('[●] ➩ الحد الأقصى لتأخير الإدخال: ',Colors.green_to_white ,interval=0.0002))
	Write.Print('[●] ➩ بعد ____ المهمة، قم بتنشيط ميزة مكافحة الحظر.',Colors.green_to_white,interval=0.0002,end='\r')
	nvblock = int(Write.Input('[●] ➩ بعد ',Colors.green_to_white,interval=0.0001))
	Write.Print(f'[●] ➩ بعد {nvblock} مهمة الراحة ____ ثانية ',Colors.green_to_white,interval=0.0002,end='\r')
	Delaybl = int(Write.Input(f'[●] ➩ بعد {nvblock} مهمة الراحة ',Colors.green_to_white,interval=0.0001))
	doinick = int(Write.Input('[●] ➩ بعد تغيير عدد الرشق: ',Colors.green_to_white,interval=0.0002))
	frive(14)
	while True:
		if len(list_cookie) == 0:
			Write.Print('تم حذف كافة كوكيز، يرجى إعادة الإدخال\n',Colors.green_to_red,interval=0.0001)
			list_cookie = Nhap_Cookie()
			with open('Cookie_FB.txt', 'w') as f:
				json.dump(list_cookie, f)
		for cookie in list_cookie:
			...
			if name != 0:
				...
				Write.Print(f'ملف تعريف ارتباط الحساب {id} Die',Colors.green_to_yellow,interval=0.0001, end='\r');sleep(3); Write.Print('                                     ', end = '\r' )
				list_cookie.remove(cookie)
				continue
			cau_hinh = tds.run(id)
			if cau_hinh == True:
				Write.Print(f'تكوين معرف FB: {id} |  اسم الفيس بوك: {ten}\n',Colors.green_to_red,interval=0.0001)
			else:
				Write.Print(f'معرف فشل التكوين FB: {id} |  اسم الفيس بوك: {ten}\n',Colors.green_to_red,interval=0.0001, end = '\r')
				continue
			ntool = 0
			while True:
			...
			######
				...
				if nvlike == 1:
					...
					if listlike == False:
						Write.Print('لا يمكن الحصول على الرشق المفضلة              ',Colors.green_to_red,interval=0.0001, end = '\r');sleep(2); print('                                                        ', end = '\r')
						nvlike = 0
					elif 'error' in listlike.text:
						if listlike.json()['error'] == 'إذا تحركت بسرعة كبيرة، يرجى إبطاء':
							coun = listlike.json()['countdown']
							Write.Print(f'الحصول على الرشق التي أعجبتك، العد التنازلي: {str(round(coun, 3))}              ',Colors.blue_to_green,interval=0.0001, end = '\r'); sleep(2); print('                                                       ', end = '\r')
						else:
						...
						if len(listlike) == 0:
							Write.Print('انتهاء عملية مثل                             ',Colors.green_to_red,interval=0.0001, end = '\r');sleep(2); print('                                                        ', end = '\r')
							nvlike = 0
						else:
							Write.Print(f'تم العثور على {len(listlike)} مهمة مثل                ',Colors.blue_to_green,interval=0.0001, end = '\r')
							for x in listlike:
								id = x['id']
								...
								if like == False:
									...
									try:
										...
										loilike = 0
										if dem % doinick == 0:
											frive(14); Write.Print(f'العملات الحالية: {xu} |  رقم حساب الفيسبوك{len(list_cookie)}\n',Colors.green_to_red,interval=0.0001); frive(14); ntool = 1; break
										if dem % nvblock == 0:
											chongblock(delaybl)
										else:
								...
								if loilike >= 5:
									name = fb.get_thongtin()
									if name == 0:
										Write.Print(f'  تم إيقاف حساب الكوكيز {ten} !!!              \n',Colors.green_to_red,interval=0.0001)
									else:
										Write.Print(f' تم حظر الحساب {ten} إعجابًا !!!					\n',Colors.green_to_red,interval=0.0001)
									list_cookie.remove(cookie)
									ntool = 1
									break
				...
				if nvlike2 == 1:
					...
					if listlike2 == False:
						Write.Print('لا يمكن الحصول على الرشق المفضلة 2                        ',Colors.green_to_red,interval=0.0001, end = '\r');sleep(2); print('                                                        ', end = '\r')
						nvlike2 = 0
					elif 'error' in listlike2.text:
						if listlike2.json()['error'] == 'إذا تحركت بسرعة كبيرة، يرجى إبطاء':
							coun = listlike2.json()['countdown']
							Write.Print(f'الحصول على مهام مثل 2، العد التنازلي: {str(round(coun, 3))}                 ',Colors.blue_to_green,interval=0.0001, end = '\r'); sleep(2); print('                                                       ', end = '\r')
						else:
						...
						if len(listlike2) == 0:
							Write.Print('انتهاء عملية مثل 2                                  ',Colors.green_to_red,interval=0.0001, end = '\r');sleep(2); print('                                                        ', end = '\r')
							nvlike2 = 0
						else:
							Write.Print(f'تم العثور على {len(listlike2)} المهمة مثل 2               ',Colors.blue_to_green,interval=0.0001, end = '\r')
							for x in listlike2:
								id = x['id']
								like = fb.like(id, 'LIKE')
								if like == False:
				...
				if nvlike3 == 1:
					...
					if listlike3 == False:
						Write.Print('لا يمكن الحصول على مهمة مثل 3                      \n',Colors.green_to_red,interval=0.0001, end = '\r');sleep(2); print('                                                        ', end = '\r')
						nvlike3 = 0
					elif 'error' in listlike3.text:
						if listlike3.json()['error'] == 'إذا تحركت بسرعة كبيرة، يرجى إبطاء':
							coun = listlike3.json()['countdown']
							Write.Print(f'الحصول على مهام مثل 3، العد التنازلي: {str(round(coun, 3))}                 ',Colors.blue_to_green,interval=0.0001, end = '\r'); sleep(2); print('                                                       ', end = '\r')
						else:
						...
						if len(listlike3) == 0:
							Write.Print('انتهاء عملية مثل 3                                 ',Colors.green_to_red,interval=0.0001, end = '\r');sleep(2); print('                                                        ', end = '\r')
							nvlike3 = 0
						else:
							Write.Print(f'تم العثور على {len(listlike3)} Quest Like 2               ',Colors.blue_to_green,interval=0.0001, end = '\r')
							for x in listlike3:
								id = x['id']
								like = fb.like(id, 'LIKE')
								if like == False:
				...
				if nvcmt == 1:
					...
					if listcmt == False:
						Write.Print('لا يمكن الحصول على مهمة التعليق                     ',Colors.green_to_red,interval=0.0001, end = '\r');sleep(2); print('                                                        ', end = '\r')
						nvcmt = 0
					elif 'error' in listcmt.text:
						if listcmt.json()['error'] == 'إذا تحركت بسرعة كبيرة، يرجى إبطاء':
							coun = listcmt.json()['countdown']
							Write.Print(f'تلقي تعليق الرشق، العد التنازلي: {str(round(coun, 3))}               ',Colors.blue_to_green,interval=0.0001, end = '\r'); sleep(2); print('                                                       ', end = '\r')
						else:
						...
						if len(listcmt) == 0:
							Write.Print('تعليق انتهاء عملية                                 \n',Colors.green_to_red,interval=0.0001, end = '\r');sleep(2); print('                                                        ', end = '\r')
							nvcmt = 0
						else:
							Write.Print(f'تم العثور على تعليق المهمة {len(listcmt)}.                      ',Colors.blue_to_green,interval=0.0001, end = '\r')
							for x in listcmt:
								id = x['id']
								msg = x['msg']
								...
								if loicmt >= 5:
									...
									if name == 0:
										...
										Write.Print(f'تم حظر الحساب {ten} بالتعليقات !!!					\n',Colors.green_to_red,interval=0.0001)
									list_cookie.remove(cookie)
									ntool = 1
									break
				...
				if nvshare == 1:
					...
					if listshare == False:
						Write.Print('لا يمكن الحصول على مهمة المشاركة                       ',Colors.green_to_red,interval=0.0001, end = '\r');sleep(2); print('                                                        ', end = '\r')
						nvshare = 0
					elif 'error' in listshare.text:
						if listshare.json()['error'] == 'إذا تحركت بسرعة كبيرة، يرجى إبطاء':
							coun = listshare.json()['countdown']
							Write.Print(f'الحصول على مشاركة الرشق، العد التنازلي: {str(round(coun, 3))}                ',Colors.blue_to_green,interval=0.0001, end = '\r'); sleep(2); print('                                                       ', end = '\r')
						else:
						...
						if len(listshare) == 0:
							Write.Print('انتهاء عملية شارك ',Colors.green_to_red,interval=0.0001, end = '\r');sleep(2); print('                                                        ', end = '\r')
							nvshare = 0
						else:
							Write.Print(f'تم العثور على {len(listshare)} مشاركة الرشق                        ',Colors.green_to_blue,interval=0.0001, end = '\r')
							for x in listshare:
								id = x['id']
								share = fb.share(id)
								if share == False:
								...
								if loishare >= 5:
									...
									if name == 0:
										...
										Write.Print(f' تم حظر الحساب {ten} شارك !!!					\n',Colors.green_to_red,interval=0.0001)
									list_cookie.remove(cookie)
									ntool = 1
									break
				...
				if nvcx == 1:
					...
					if listcx == False:
						Write.Print('لا يمكن الحصول على الرشق البنجاح                             ',Colors.green_to_red,interval=0.0001, end = '\r');sleep(2); print('                                                        ', end = '\r')
						nvcx = 0
					elif 'error' in listcx.text:
						if listcx.json()['error'] == 'إذا تحركت بسرعة كبيرة، يرجى إبطاء':
							coun = listcx.json()['countdown']
							Write.Print(f'الحصول على الرشق البنجاح، العد التنازلي: {str(round(coun, 3))}                 ',Colors.green_to_blue,interval=0.0001, end = '\r'); sleep(2); print('                                                       ', end = '\r')
						else:
						...
						listcx = listcx.json()
						if len(listcx) == 0:
							Write.Print('شعور انتهاء عملية Xúc                                 \n',Colors.green_to_red,interval=0.0001, end = '\r');sleep(2); print('                                                        ', end = '\r')
							nvcx = 0
						else:
							Write.Print(f'تم العثور على {len(listcx)} مهمة بنجاح                       ',Colors.green_to_blue,interval=0.0001, end = '\r')
							for x in listcx:
								id = x['id']
								type = x['type']
								...
								if loilike >= 5:
									...
									if name == 0:
										...
										Write.Print(f' {ten} تم حظر الحساب تم كشفك!!!				\n',Colors.green_to_red,interval=0.0001)
									list_cookie.remove(cookie)
									ntool = 1
									break
				...
				if nvfollow == 1:
					...
					if listfollow == False:
						Write.Print('لا يمكن الحصول على متابعة المهمة                      ',Colors.green_to_red,interval=0.0001, end = '\r');sleep(2); print('                                                        ', end = '\r')
						nvfollow = 0
						listfollow = []
					elif 'error' in listfollow.text:
						if listfollow.json()['error'] == 'إذا تحركت بسرعة كبيرة، يرجى إبطاء':
							coun = listfollow.json()['countdown']
							Write.Print(f'متابعة الرشق، العد التنازلي: {str(round(coun, 3))}                     ',Colors.green_to_blue,interval=0.0001, end = '\r'); sleep(2); print('                                                       ', end = '\r')
						else:
						...
						if len(listfollow) == 0:
							Write.Print('اتبع انتهاء عملية                                    \n',Colors.green_to_red,interval=0.0001, end = '\r');sleep(2); print('                                                        ', end = '\r')
							nvfollow = 0
						else:
							Write.Print(f'تم العثور على {len(listfollow)} متابعة المهمة       \n',Colors.green_to_blue,interval=0.0001, end = '\r')
							for x in listfollow:
								id = x['id']
								follow = fb.follow(id)
								if follow == False:
								...
								if loifollow >= 5:
									...
									if name == 0:
										...
										Write.Print(f' تم حظر الحساب {ten} !!!			\n',Colors.green_to_red,interval=0.0001)
									list_cookie.remove(cookie)
									ntool = 1
									break
				...
				if nvpage == 1:
					...
					if listpage == False:
						Write.Print('لا يمكن الحصول على مهمة الإعجاب بالصفحة                 ',Colors.green_to_red,interval=0.0001, end = '\r');sleep(2); print('                                                        ', end = '\r')
						nvpage = 0
						listpage = []
					elif 'error' in listpage.text:
						if listpage.json()['error'] == 'هذه العملية سريعة جدًا، يرجى إبطاء السرعة':
							 coun = listpage.json()['countdown']
							 Write.Print(f'الحصول على صفحة الرشق، COUNTDOWN: {str(round(coun, 3))} ',Colors.green_to_blue,interval=0.0001, end = '\r'); sleep(2); print('                                                       ', end = '\r')
						...
						if len(listpage) == 0:
							Write.Print('انتهت مهمة الإعجاب بالصفحة                               \n',Colors.green_to_red,interval=0.0001, end = '\r');sleep(2); print('                                                        ', end = '\r')
							nvpage = 0
						else:
							Write.Print(f'تم العثور على {len(listpage)} صفحة الإعجاب بالمهمة        \n',Colors.green_to_blue,interval=0.0001, end = '\r')
							for x in listpage:
								id = x['id']
								page = fb.page(id)
								if page == False:
								...
								if loipage >= 5:
									...
									if name == 0:
										...
										Write.Print(f' تم حظر الحساب {ten} الإعجاب بالصفحة !!!					\n',Colors.green_to_red,interval=0.0001)
									list_cookie.remove(cookie)
									ntool = 1
									break
				if ntool == 1:
				...
				if nvcxcmt == 1:
					...
					if listcxcmt == False:
						Write.Print('غير قادر على الحصول على الرشق تعليق الحساب',Colors.green_to_red,interval=0.0001, end = '\r');sleep(2); print('                                                        ', end = '\r')
						nvcxcmt = 0
						listcxcmt = []
					elif 'error' in listcxcmt.text:
						if listcxcmt.json()['error'] == 'إذا تحركت بسرعة كبيرة، يرجى إبطاء':
							coun = listcxcmt.json()['countdown']
							Write.Print(f'تلقي تعليق الرشق البنجاح، العد التنازلي: {str(round(coun, 3))}                   ',Colors.green_to_blue,interval=0.0001, end = '\r'); sleep(2); print('                                                       ', end = '\r')
						else:
						...
						if len(listcxcmt) == 0:
							Write.Print('تعليق حساب انتهاء عملية                ',Colors.green_to_red,interval=0.0001, end = '\r');sleep(2); print('                                                        ', end = '\r')
							listcxcmt = 0
						else:
							Write.Print(f'تم العثور على {len(listcxcmt)} تعليق حساب المهمة             ',Colors.green_to_blue,interval=0.0001, end = '\r')
							for x in listcxcmt:
								id = x['id']
								type = x['type']
								...
								if loicxcmt >= 5:
									...
									if name == 0:
										...
										Write.Print(f' تم حظر حساب {ten} بواسطة CMT Emotions !!!					\n',Colors.green_to_red,interval=0.0001)
									list_cookie.remove(cookie)
									ntool = 1
									break
				if ntool == 1:
				...
				if nvgroup == 1:
					...
					if listgroup == False:
						Write.Print('لا يمكن الحصول على مهمة جماعية                               ',Colors.green_to_red,interval=0.0001, end = '\r');sleep(2); print('                                                        ', end = '\r')
						nvgroup = 0
						listgroup = []
					elif 'error' in listgroup.text:
						if listgroup.json()['error'] == 'Thao العمل سريع جدًا، يرجى الإبطاء':
							coun = listgroup.json()['countdown']
							Write.Print(f'الحصول على مجموعة الرشق، العد التنازلي: {str(round(coun, 3))}               ',Colors.green_to_blue,interval=0.0001, end = '\r'); sleep(2); print('                                                       ', end = '\r')
						else:
							print(listlike.json()['error'] , end ='\r')
						nvgroup = 0
						listgroup = []
						...
						if len(listgroup) == 0:
							Write.Print('نهاية مهمة المجموعة                                ',Colors.green_to_red,interval=0.0001, end = '\r');sleep(2); print('                                                        ', end = '\r')
							listgroup = 0
						else:
							Write.Print('تم العثور على مجموعة الرشق {len(listgroup)}.           \n',Colors.green_to_blue,interval=0.0001, end = '\r')
							...
							for x in listgroup:
								...
								if loigroup >= 5:
									...
									if name == 0:
										...
										Write.Print(f' تم حظر الحساب {ten}، انضم إلى المجموعة !!!					\n',Colors.green_to_red,interval=0.0001)
									list_cookie.remove(cookie)
									ntool = 1
									break
				if ntool == 1:
				...
				if nvcx + nvgroup + nvcxcmt + nvpage + nvfollow + nvshare + nvcmt + nvlike == 0:
					for i in range(10, 0, -1):
						Write.Print(f' انتهت صلاحية جميع الرشق، يرجى الانتظار {i} ثانية ',Colors.green_to_red,interval=0.0001, end = '\r');sleep(1); print('                                                        ', end = '\r')
